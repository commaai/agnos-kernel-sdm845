...
static int tcan4x5x_parse_config(struct m_can_classdev *cdev)
{
	...
	tcan4x5x->device_wake_gpio =
		devm_gpiod_get_optional(cdev->dev, "device-wake",
					GPIOD_OUT_HIGH);
	if (IS_ERR(tcan4x5x->device_wake_gpio))
		return PTR_ERR(tcan4x5x->device_wake_gpio);

	tcan4x5x->reset_gpio = devm_gpiod_get_optional(cdev->dev, "reset",
						       GPIOD_OUT_LOW);
	if (IS_ERR(tcan4x5x->reset_gpio))
		return PTR_ERR(tcan4x5x->reset_gpio);

	tcan4x5x->device_state_gpio = devm_gpiod_get_optional(cdev->dev,
							      "device-state",
							      GPIOD_IN);
	if (IS_ERR(tcan4x5x->device_state_gpio))
		return PTR_ERR(tcan4x5x->device_state_gpio);
...
static int tcan4x5x_can_probe(struct spi_device *spi)
{
	 ...
	 */
	ret = regmap_read(priv->regmap, TCAN4X5X_DEV_ID1, &dev_id);
	if (ret)
		goto out_power;
...
/*
 * m_can_class_register() releases the CAN netdevice itself on failure.
 */
out_power_registered:
	tcan4x5x_power_enable(priv->power, 0);
	spi_set_drvdata(spi, NULL);
	dev_err(&spi->dev, "Probe failed, err=%d\n", ret);
	return ret;
