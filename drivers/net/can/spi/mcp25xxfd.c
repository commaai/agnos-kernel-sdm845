...
static int mcp25xxfd_can_probe(struct spi_device *spi)
{
	...
	//ret = clk_set_rate(clk, 1000000);
	//if (ret)
	//	dev_err(&spi->dev, "%s: clk_set_rate failed %d\n",
	//						__func__, ret);
	...
	dev_err(&spi->dev, "spi_setup_speed_hz = %d\n", priv->spi_setup_speed_hz);
...
{
	int ret;
	printk(KERN_ERR "mcp25xxfd_can_driver_init\n");
