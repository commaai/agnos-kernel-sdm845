...
static int is_directory(const char *base_path, const struct dirent *dent)
{
	...
	assert(snprintf(path, PATH_MAX, "%s/%s", base_path, dent->d_name));
	if (stat(path, &st))
		return 0;
				  ...
				  int unset __maybe_unused)
{
	...
	for_each_lang(scripts_path, scripts_dir, lang_dirent) {
		assert(snprintf(lang_path, MAXPATHLEN, "%s/%s/bin", scripts_path,
			 lang_dirent->d_name));
		...
		for_each_script(lang_path, lang_dir, script_dirent) {
			...
			if (script_root) {
				...
				assert(snprintf(script_path, MAXPATHLEN, "%s/%s",
			...
			struct perf_session *session)
{
	...
	assert(snprintf(filename, MAXPATHLEN, "%s/bin/%s-record", dir_name, scriptname));
...
int find_scripts(char **scripts_array, char **scripts_path_array)
{
	...
	for_each_lang(scripts_path, scripts_dir, lang_dirent) {
		assert(snprintf(lang_path, MAXPATHLEN, "%s/%s", scripts_path,
			 lang_dirent->d_name));
#ifdef NO_LIBPERL
